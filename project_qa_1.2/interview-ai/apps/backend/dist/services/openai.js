...
class OpenAIService {
    constructor() {
        ...
        if (this.apiKey) {
            ...
            logger_1.logger.warn('⚠️ OpenAI API key not found. AI features will be disabled.');
        }
    }
    /**
     * Generar pregunta de entrevista contextual
     */
    async generateInterviewQuestion(jobRole, experienceLevel, previousQuestions = []) {
        if (!this.client) {
...
`;
        try {
            ...
            logger_1.logger.error('Error generating interview question:', error);
     ...
     */
    async evaluateInterviewResponse(question, answer, jobRole, experienceLevel) {
        if (!this.client) {
            throw new Error('OpenAI client not initialized');
        }
        const prompt = `
...
`;
        try {
            ...
            try {
                ...
                logger_1.logger.error('Error parsing OpenAI response:', parseError);
                // Fallback response
                return {
                    overallScore: 7,
                    categories: {
            ...
            logger_1.logger.error('Error evaluating interview response:', error);
     ...
     */
    async generateOverallFeedback(responses, jobRole) {
        if (!this.client) {
            throw new Error('OpenAI client not initialized');
        }
        const prompt = `
...
`;
        try {
            ...
            logger_1.logger.error('Error generating overall feedback:', error);
