...
import { useAuthStore } from '../store/authStore' // ← usamos el store real

const LoginPage = () => {
  const [username, setUsername] = useState('')   // lo usamos como email
  const [password, setPassword] = useState('')
  const [loading, setLoading] = useState(false)
  const navigate = useNavigate()
  const login = useAuthStore((s) => s.login)     // ← función real que llama a la API
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const { success, error } = await login(username, password) // ← valida en backend
    setLoading(false)
    ...
    if (success) {
      ...
      // Importante: NO navegamos si falla
    }
  }
